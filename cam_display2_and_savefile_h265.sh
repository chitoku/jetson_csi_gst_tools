...
COMMAND="gst-launch-1.0 nvarguscamerasrc ${CMD_SENSORID} ${CMD_SENSORMODE} ${CMD_TIMEOUT} ${CMD_EXPOSURERANGE} ! \
  ...
  tee name=t \
  t. ! queue ! nvvidconv flip-method=7 ! nvoverlaysink sync=false -e \
  t. ! queue leaky=2 max-size-buffers=1 ! nvvidconv ${CMD_FLIPMETHOD} ! nvv4l2h265enc ${CMD_BITRATE} ! h265parse ! qtmux ! filesink location=$FILENAME -e"
